...
# ─── Phase 0.5-D-e-2: LLM client pool 分離テスト ──────────────────


class TestLlmClientPool:
    """Phase 0.5-D-e-2: bg_runner / fallback の OpenAI client pool 分離。

    中間実走 11 回目 take 3 で観察された 5 分以上 hang の核心 (= 同一 model + 同一
    session_id で ChatOpenAI を毎回新規生成 → httpx connection pool 競合 → 二重
    hang) への構造的対処。session_id + mode を key に LLM client を pool 化する。
    """

    @pytest.fixture(autouse=True)
    def _clear_pool(self):
        """各テスト前後で client pool を空に保ち、テスト間漏れを防ぐ。"""
        from lab_lounge import graph as graph_mod
        graph_mod._clear_llm_client_pool()
        yield
        graph_mod._clear_llm_client_pool()

    def test_returns_same_for_same_session_mode(self, monkeypatch):
        """同じ (session_id, mode) では同じ client インスタンスが返る。"""
        from lab_lounge import graph as graph_mod

        created = []

        def fake_create(provider, model):
            obj = MagicMock(name=f"client-{len(created)}")
            created.append(obj)
            return obj

        monkeypatch.setattr(graph_mod, "_create_llm_client_raw", fake_create)

        c1 = graph_mod._get_or_create_llm_client(
            "openai", "gpt-test", session_id="sess-A", mode="bg",
        )
        c2 = graph_mod._get_or_create_llm_client(
            "openai", "gpt-test", session_id="sess-A", mode="bg",
        )
        assert c1 is c2
        assert len(created) == 1, "同 key で 2 度生成された (= pool が機能していない)"

    def test_returns_different_for_different_modes(self, monkeypatch):
        """同 session でも mode が違えば別 client が返る (= bg と normal の分離)。"""
        from lab_lounge import graph as graph_mod

        def fake_create(provider, model):
            return MagicMock(name=f"client-{provider}-{model}")

        monkeypatch.setattr(graph_mod, "_create_llm_client_raw", fake_create)

        c_bg = graph_mod._get_or_create_llm_client(
            "openai", "gpt-test", session_id="sess-A", mode="bg",
        )
        c_normal = graph_mod._get_or_create_llm_client(
            "openai", "gpt-test", session_id="sess-A", mode="normal",
        )
        assert c_bg is not c_normal, (
            "bg と normal で同 client が共有された "
            "(= connection pool 競合解消が無効)"
        )

    def test_returns_different_for_different_sessions(self, monkeypatch):
        """異なる session_id では別 client が返る (= ターン間の漏れ防止)。"""
        from lab_lounge import graph as graph_mod
        ...
        c2 = graph_mod._get_or_create_llm_client(
            "openai", "gpt-test", session_id="sess-B", mode="bg",
        )
        assert c1 is not c2

    def test_no_pool_for_empty_session(self, monkeypatch):
        """session_id="" では pool 化せず毎回新規生成 (= 後方互換)。"""
        from lab_lounge import graph as graph_mod
        ...
        c1 = graph_mod._get_or_create_llm_client(
            "openai", "gpt-test", session_id="", mode="normal",
        )
        c2 = graph_mod._get_or_create_llm_client(
            "openai", "gpt-test", session_id="", mode="normal",
        )
        assert c1 is not c2
        assert len(created) == 2, "session_id 空でも pool 化された (= 後方互換崩壊)"

    def test_cleanup_removes_session(self, monkeypatch):
        """_cleanup_llm_client_pool(session) で該当 session の client を全 mode 削除。"""
        from lab_lounge import graph as graph_mod
        ...
        graph_mod._get_or_create_llm_client(
            "openai", "gpt-test", session_id="sess-A", mode="bg",
        )
        graph_mod._get_or_create_llm_client(
            "openai", "gpt-test", session_id="sess-A", mode="normal",
        ...
        assert ("sess-A", "bg") in graph_mod._llm_client_pool
        assert ("sess-A", "normal") in graph_mod._llm_client_pool
        assert ("sess-B", "bg") in graph_mod._llm_client_pool

        graph_mod._cleanup_llm_client_pool("sess-A")

        assert ("sess-A", "bg") not in graph_mod._llm_client_pool
        assert ("sess-A", "normal") not in graph_mod._llm_client_pool
        # 他 session は残る
        assert ("sess-B", "bg") in graph_mod._llm_client_pool

    def test_cleanup_except_keeps_only_active(self, monkeypatch):
        """_cleanup_llm_client_pool_except(active) で active 以外を削除。

        本番経路: ask_character.set_ask_character_context が新ターン開始時に
        呼出して、前ターンまでの client を解放する仕組み。
        """
        from lab_lounge import graph as graph_mod
        ...
        graph_mod._get_or_create_llm_client(
            "openai", "gpt-test", session_id="sess-old1", mode="bg",
        )
        graph_mod._get_or_create_llm_client(
            "openai", "gpt-test", session_id="sess-old2", mode="normal",
        )
        graph_mod._get_or_create_llm_client(
            "openai", "gpt-test", session_id="sess-current", mode="bg",
        )

        graph_mod._cleanup_llm_client_pool_except("sess-current")

        assert ("sess-old1", "bg") not in graph_mod._llm_client_pool
        assert ("sess-old2", "normal") not in graph_mod._llm_client_pool
        # active は保持
        assert ("sess-current", "bg") in graph_mod._llm_client_pool

    def test_get_llm_for_agent_uses_contextvars(self, monkeypatch):
        """_get_llm_for_agent は contextvars から session_id + mode を読む。

        pipeline.run_pipeline / run_pipeline_llm_only がそれぞれ "normal" / "bg"
        を set し、_get_llm_for_agent がそれを読み取って pool key とする経路を
        検証する。
        """
        from lab_lounge import graph as graph_mod

        def fake_create(provider, model):
            return MagicMock(name="raw-client")

        monkeypatch.setattr(graph_mod, "_create_llm_client_raw", fake_create)

        # contextvars に "bg" / session_id をセット
        sid_token = graph_mod._llm_client_session_id_var.set("sess-X")
        mode_token = graph_mod._llm_client_mode_var.set("bg")
        try:
            client = graph_mod._get_llm_for_agent("openai", "gpt-test")
        finally:
            graph_mod._llm_client_session_id_var.reset(sid_token)
            graph_mod._llm_client_mode_var.reset(mode_token)

        assert ("sess-X", "bg") in graph_mod._llm_client_pool
        assert graph_mod._llm_client_pool[("sess-X", "bg")] is client
